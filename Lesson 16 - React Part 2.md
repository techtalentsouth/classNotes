...
class App extends Component {
  constructor() {
     super(); //Call the parent object for initialization
     this.state = { //start building the state object
   		clickCount : 0
   	};
   }
...
This will utilize the React lifecycle (discussed next class) and allow React to keep track of the fact that state is changing!
...
In the above example, We have simply setup our single HelloMessage component to receive props. We then pass the props on our call to instantiate the component. 

### Passing Props to Children
let messageListTemp = this.state.messageList;
    messageListTemp.push(messageValue);
...
class TodoList extends React.Component {
  ...
  render() {
    ...
    let items = this.state.listItems.map((item,i) => {
      return <TodoItem key={i} todo={item} completeItem={this.completeItem} />
    })
